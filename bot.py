...
async def sys(ctx, *, arg):
   ...
   """
    ...
    if "fortunate one" in [y.name.lower() for y in ctx.message.author.roles]:  # |•| CHANGE ROLE TO TWITCH SUB or list
        # ATTEMPT SUBMISSION
        try:
...
def list_submissions(sql2, ctx):
    ...
    try:
        ...
        print(e_desc)
        embed_list = discord.Embed(color=discord.Color.darker_gray(), description=e_desc)
